...
\begin{sectioncontent}
    \hspace{\parindent}Penelitian ini menggunakan pendekatan eksperimen komparatif. Pendekatan tersebut digunakan karena penelitian ini membandingkan dua model \textit{deep learning}, yaitu YOLOv8 Nano dan YOLO11 Nano, pada tugas klasifikasi penyakit daun tomat. Perbandingan dilakukan berdasarkan dua aspek utama, yaitu akurasi klasifikasi dan efisiensi inferensi ketika model dijalankan pada perangkat \textit{edge}.

    Eksperimen dilakukan dengan menggunakan dataset dan konfigurasi pengujian yang sama untuk kedua model. Dengan cara tersebut, perbedaan hasil yang diperoleh dapat lebih diarahkan pada perbedaan performa model, bukan pada perbedaan data atau skenario pengujian. Hasil dari penelitian ini kemudian digunakan untuk menentukan model yang lebih sesuai untuk diterapkan pada sistem klasifikasi penyakit daun tomat berbasis perangkat \textit{edge}.
\end{sectioncontent}
...
\begin{sectioncontent}
    ...
    \begin{enumerate}[leftmargin=0.75cm,label=\arabic*.]
        ...
        \item Membagi dataset ke dalam data latih, data validasi, dan data uji.
        \item Melakukan \textit{preprocessing} dataset agar sesuai dengan kebutuhan model klasifikasi YOLO.
        \item Melatih model YOLOv8 Nano dan YOLO11 Nano menggunakan konfigurasi yang sama.
        \item Mengevaluasi performa klasifikasi model menggunakan data validasi dan data uji.
        ...
        \item Menganalisis perbandingan akurasi dan efisiensi kedua model.
...
\begin{sectioncontent}
    \hspace{\parindent}Dataset yang digunakan dalam penelitian ini merupakan dataset publik citra penyakit daun tomat. Dataset tersebut berisi gambar daun tomat yang telah dikelompokkan ke dalam beberapa kelas penyakit dan kelas daun sehat. Penggunaan dataset publik dipilih agar penelitian dapat dilakukan dengan data yang telah tersedia, memiliki label kelas yang jelas, serta dapat disusun sesuai kebutuhan tugas klasifikasi citra.
...
|-- test/
    ...
    Pembagian dataset dilakukan dengan memperhatikan proporsi data pada setiap kelas. Data latih digunakan untuk melatih model, data validasi digunakan untuk memantau performa model selama proses pelatihan, sedangkan data uji digunakan untuk mengevaluasi performa akhir model. Data uji tidak digunakan dalam proses pelatihan maupun pemilihan parameter model agar hasil evaluasi lebih objektif.
\end{sectioncontent}
...
\begin{sectioncontent}
    ...
    \begin{subsectioncontent}
        ...
        \begin{enumerate}[leftmargin=0.75cm,label=\arabic*.]
            \item Komputer atau laptop untuk proses pelatihan model.
...
\begin{sectioncontent}
    \hspace{\parindent}\textit{Preprocessing} dilakukan untuk memastikan data citra siap digunakan dalam proses pelatihan dan pengujian model. Tahapan ini meliputi pemeriksaan dataset, penyesuaian struktur folder, pembagian dataset, serta penyesuaian ukuran citra.

    Pada penelitian ini, citra diproses dengan ukuran masukan 224 x 224 piksel. Ukuran tersebut digunakan karena sesuai dengan skenario klasifikasi citra pada model YOLO versi klasifikasi. Selain itu, normalisasi nilai piksel dilakukan agar citra dapat diproses oleh model secara konsisten.

    Tahapan \textit{preprocessing} dalam penelitian ini meliputi:
...
\begin{sectioncontent}
    \hspace{\parindent}Pelatihan model dilakukan menggunakan dataset yang sama dan konfigurasi pelatihan yang dibuat seragam untuk kedua model. Penggunaan konfigurasi yang sama bertujuan agar perbandingan antara YOLOv8 Nano dan YOLO11 Nano dapat dilakukan secara adil. Parameter utama seperti ukuran citra, jumlah epoch, dan \textit{batch size} dibuat sama pada kedua model.

    \newpage
    Konfigurasi pelatihan yang digunakan adalah sebagai berikut:
    ...
    \hspace{\parindent}Perintah pelatihan yang digunakan adalah sebagai berikut:
...
epochs=50 imgsz=224 batch=16 device=0
    ...
    Kedua model dilatih dengan konfigurasi yang sama agar hasil perbandingan lebih konsisten. Apabila terdapat perbedaan performa pada tahap evaluasi, perbedaan tersebut dapat dianalisis sebagai hasil dari karakteristik masing-masing model, bukan akibat perbedaan konfigurasi pelatihan.
\end{sectioncontent}
...
\begin{sectioncontent}
    ...
    \begin{subsectioncontent}
        ...
        \begin{equation}
            Accuracy = \frac{Jumlah\ Prediksi\ Benar}{Jumlah\ Seluruh\ Data\ Uji}
        \end{equation}
    \end{subsectioncontent}
    ...
    \begin{subsectioncontent}
        ...
        \hspace{\parindent}Keterangan $K$ menunjukkan jumlah kelas yang digunakan dalam penelitian. Dengan menggunakan \textit{macro average}, setiap kelas memiliki pengaruh yang sama terhadap nilai akhir, sehingga hasil evaluasi tidak hanya mencerminkan performa pada kelas yang jumlah datanya lebih besar atau lebih mudah dikenali.
    \end{subsectioncontent}
...
\begin{sectioncontent}
    \hspace{\parindent}Analisis data dilakukan dengan membandingkan hasil pengujian YOLOv8 Nano dan YOLO11 Nano berdasarkan metrik yang telah ditentukan. Hasil pengujian klasifikasi dianalisis menggunakan akurasi, \textit{macro precision}, \textit{macro recall}, \textit{macro F1-score}, hasil evaluasi per kelas, dan \textit{confusion matrix}. Sementara itu, hasil pengujian efisiensi pada perangkat \textit{edge} dianalisis berdasarkan waktu inferensi, FPS, penggunaan memori, dan ukuran model.

    Hasil pengujian kedua model disajikan dalam bentuk tabel dan grafik agar perbandingan lebih mudah dianalisis. Model yang memiliki akurasi lebih tinggi dan waktu inferensi lebih rendah dinilai lebih sesuai untuk diterapkan pada sistem klasifikasi penyakit daun tomat berbasis perangkat \textit{edge}. Namun, apabila terdapat perbedaan antara akurasi dan efisiensi, analisis dilakukan dengan mempertimbangkan keseimbangan antara kedua aspek tersebut.
\end{sectioncontent}
...
\begin{sectioncontent}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{|l|c|c|c|c|}
            \hline
            \textbf{Model} & \textbf{Akurasi} & \textbf{Macro Precision} & \textbf{Macro Recall} & \textbf{Macro F1-score} \\
            \hline
            YOLOv8 Nano & - & - & - & - \\
            \hline
            YOLO11 Nano & - & - & - & - \\
    ...
    Tabel tersebut digunakan sebagai dasar untuk membandingkan performa kedua model. Penentuan model yang lebih sesuai tidak hanya didasarkan pada akurasi tertinggi, tetapi juga mempertimbangkan efisiensi inferensi ketika model dijalankan pada perangkat \textit{edge}.
\end{sectioncontent}